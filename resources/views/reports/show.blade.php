...
<head>
    ...
    <style>
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
        body{font-family:'Sora',sans-serif; background-image: linear-gradient(to right, rgba(22, 45, 110, 0.15) 1px, transparent 1px), linear-gradient(to bottom, rgba(22, 45, 110, 0.15) 1px, transparent 1px);background-size: 40px 40px; color:var(--text);}
        ...
        /* UPVOTE */
        .upvote-hero{text-align:center;padding:24px;background:var(--bg);border:2px dashed var(--accent);border-radius:12px;}
...
<body>
    ...
    <div class="detail-wrap">
        ...
        <div class="report-main">
            <!-- IMAGE -->
            @if($report->image)
                <div x-data="{ showModal: false }" style="width: 100%; height: 100%;">
                    <img src="{{ asset('storage/'.$report->image) }}" alt="{{ $report->title }}" class="report-img" @click="showModal = true" style="cursor: zoom-in;" title="Klik untuk memperbesar">
                    <div x-show="showModal" style="display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background-color: rgba(0, 0, 0, 0.85); z-index: 99999;" x-transition.opacity @keydown.escape.window="showModal = false" @click="showModal = false"> 
                        <button type="button" @click="showModal = false" style="position: absolute; top: 20px; right: 24px; background: transparent; border: none; color: #fff; font-size: 40px; cursor: pointer; z-index: 100000; line-height: 1;"> &times; </button>

                        <img src="{{ asset('storage/'.$report->image) }}" alt="{{ $report->title }}" @click.stop style="position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); max-width: 90vw; max-height: 90vh; border-radius: 8px; box-shadow: 0 10px 25px rgba(0,0,0,0.5); object-fit: contain;">
                    </div>
                </div>
            @else
